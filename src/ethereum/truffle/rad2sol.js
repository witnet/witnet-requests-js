...
const {
  ...
  requestsBanner, requestsSucceed, writeMigrations, writeMigrationsRadons, writeSol
...
const steps = [
  ...
  (hex, i) => {
    const priceName = requestNames[i].replace(/\.js/, "")
    migrationsRadons[`${priceName}Feed`] = {
      ERC2362ID: 
        `Price-${priceName.replace("Price", "")
          .split(/(?=[A-Z])/)
          .map(s => s.toUpperCase())
          .join("/")
        }-3`,
...
Promise.all(
  ...
  .then(() => writeMigrationsRadons(migrationsRadons, migrationsDir, fs))
